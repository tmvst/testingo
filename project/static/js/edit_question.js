...
(function() {
  var check_template, delete_entry, form_submit, get_cnt, input_template, ix, phrase_template, radio_template, update_cnt;
  ...
  update_cnt = function(index_of_deleted) {
    $("input.text").each(function() {
      var current_index;
      current_index = parseInt($(this).attr("name".substr(5)));
      if (current_index > index_of_deleted) {
        return $(this).attr("name", "text" + current_index);
      }
    });
    return $("input.Rradio").each(function() {
      var current_index;
      current_index = parseInt($(this).val().substr(3));
      if (current_index > index_of_deleted) {
        return $(this).attr("value", "ind" + current_index);
      }
    });
  };
