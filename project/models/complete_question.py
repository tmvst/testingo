...
class CompleteQuestion(Base):
    ...
    incomplete_test = relationship('Incomplete_test', backref=backref("complete_questions",order_by=id.asc(), cascade="all, delete-orphan"))
    question_id = Column(Integer, ForeignKey('question.id'), index=True)
    question = relationship('Question', backref=backref("complete_question",order_by=id.asc(), cascade="all, delete-orphan"))
